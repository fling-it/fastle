...
```bash
npm exec fling -- --cli push        # Safe from any environment
```
...
```bash
npm exec fling -- --cli signup --email user@example.com --name "User Name"
```

**Existing user:** Log in via magic link email:
```bash
npm exec fling -- --cli login --email user@example.com
```
...
npm exec fling upgrade          # to upgrade this skill if needed
